    ...
    <div className="" >
        ...
        <CardFooter className="flex justify-between border-t mt-1 pt-3">
            <div className="flex justify-between items-center mb-2">
                <span className="text-md text-secondary font-bold  dark:text-primary ">{product?.category}</span>
            </div>
            <div className="flex justify-between items-center gap-3 mb-2">
                {product?.open ? <CheckCheck className="text-green-500  size-5" /> : <Ban className="text-red-800  size-5" />}
                <span className={`${product?.open ? "text-secondary  dark:text-primary " : 'text-red-500'}`}>{product?.open ? "Open" : 'Close'}</span>
            </div>
        </CardFooter>
    </div>
